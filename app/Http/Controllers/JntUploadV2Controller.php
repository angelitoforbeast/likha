    ...
    public function status(Request $request, int $runId)
    {
        ...
        return response()->json([
            'run' => [
                ...
                'last_batch_at'            => optional($run->last_batch_at)->toDateTimeString(),
                'last_batch_duration_ms'   => $run->last_batch_duration_ms !== null ? (int) $run->last_batch_duration_ms : null,
                'seconds_since_last_batch' => $run->last_batch_at !== null ? max(0, Carbon::now('Asia/Manila')->timestamp - Carbon::parse($run->last_batch_at->format('Y-m-d H:i:s'), 'Asia/Manila')->timestamp) : null,
