<x-layout>
  ...
  <div class="max-w-6xl mt-6 space-y-6">
    ...
    {{-- MULTIPLE UPLOAD --}}
    <form action="{{ route('pancake.conversations.store') }}"
          method="POST"
      ...
      <div>
        <label class="block font-semibold mb-1">Upload files (CSV / XLSX / XLS / ZIP)</label>
        <input type="file"
               name="files[]"
    ...
    <div class="border rounded p-4 bg-gray-50">
      ...
      <div class="grid grid-cols-2 gap-4 mt-3">
        <div>
          <div class="text-sm text-gray-600">Inserted (new unique page+name)</div>
          <div id="latest-inserted" class="text-lg font-semibold">{{ $inserted }}</div>
        </div>
        <div>
          <div class="text-sm text-gray-600">Updated (existing appended)</div>
  ...
  (function(){
    ...
    async function pollOnce() {
      try {
        const res = await fetch("{{ route('pancake.conversations.status') }}", {
          headers: { 'X-Requested-With': 'XMLHttpRequest' }
        });
        const data = await res.json();
        const logs = data.logs || [];

        const knownIds = new Set([...document.querySelectorAll('tr[data-log-id]')]
          .map(r => parseInt(r.dataset.logId)));
