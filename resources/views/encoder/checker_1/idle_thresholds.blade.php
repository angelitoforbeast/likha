...
<head>
  <meta charset="utf-8" />
  <title>Checker 1 – Idle Thresholds</title>
...
<body class="text-gray-900">
  ...
  <nav class="bg-white border-b sticky top-0 z-50">
    <div class="max-w-4xl mx-auto px-4">
      <div class="h-14 flex items-center justify-between">
        <div class="font-semibold text-lg">Checker 1 – Idle Thresholds</div>
  ...
  <main class="max-w-3xl mx-auto px-4 py-6 space-y-4">
    ...
    @if($saved)
      <div class="bg-green-50 border border-green-200 text-green-800 rounded-lg p-3 text-sm">
        ✓ Thresholds saved. Refresh the Idle Summary page to apply.
      </div>
    @endif
    ...
    <section class="bg-white rounded-xl shadow p-5">
      <div class="font-semibold text-lg mb-1">⚙ Activity Thresholds (Global)</div>
      <p class="text-sm text-gray-500 mb-5">
        ...
        classified into <strong>Working / Idle / Long break / Away</strong> buckets. Setting takes effect for every viewer of
        the Idle Summary page after save.
      </p>

      <form method="POST" action="{{ route('encoder.checker1.idle-thresholds.update') }}" class="space-y-5">
        @csrf
        ...
        <div class="border-t pt-4 flex items-center gap-2">
          ...
          <button type="button"
                  onclick="document.getElementById('work').value={{ $defaults['work'] }};
  ...
  <script>
    // Live preview of seconds → "Xm Ys" or "Xh Ym" under each input.
    function fmtSec(s) {
      s = Number(s) || 0;
      if (s < 60) return s + 's';
      if (s < 3600) {
