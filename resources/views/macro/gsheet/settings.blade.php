<x-layout>
  ...
  <div class="max-w-4xl mx-auto mt-6">
    ...
    <form method="POST" action="{{ route('macro.settings.store') }}" class="space-y-4 bg-white p-4 rounded shadow mb-6">
      @csrf
      <div>
        <label class="text-sm font-medium">Gsheet Name (optional)</label>
        <input type="text" name="gsheet_name" class="w-full border rounded px-3 py-2 mt-1">
      </div>
      <div>
              ...
              <tr>
                <form method="POST" action="{{ route('macro.settings.update', $setting->id) }}">
                  ...
                  <td class="border px-4 py-2">
                    <input type="text" name="gsheet_name" value="{{ $setting->gsheet_name }}" class="w-full border rounded px-2 py-1">
                  </td>
                  <td class="border px-4 py-2">
