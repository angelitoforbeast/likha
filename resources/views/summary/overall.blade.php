    ...
    <section class="bg-white rounded-xl shadow p-4">
      <div class="flex items-center justify-between mb-3">
        ...
        <div class="text-xs text-gray-500">
          ...
          (Adspent, Orders, Proceed, Cannot Proceed, ODZ, Shipped, <b>Delivered</b>, TCPR)
        </div>
      </div>
        ...
        <table class="min-w-[1120px] w-full text-sm">
          <thead class="bg-gray-50">
            <tr class="text-left text-gray-600">
              ...
              <th class="px-3 py-2">Adspent</th>
              <th class="px-3 py-2">Orders</th>
              <th class="px-3 py-2">Proceed</th>
              <th class="px-3 py-2">Cannot Proceed</th>
          ...
          <tbody>
            <template x-if="!data.ads_daily || data.ads_daily.length===0">
              <tr class="border-t">
                <td class="px-3 py-3 text-gray-500" colspan="10">No data for selected filters.</td>
              </tr>
            </template>
            <template x-for="row in data.ads_daily" :key="row.date + '|' + (row.page ?? '')">
              <tr class="border-t hover:bg-gray-50">
                <td class="px-3 py-2" x-text="row.date"></td>
                <td class="px-3 py-2" x-text="row.page ?? '—'"></td>
                <td class="px-3 py-2" x-text="money(row.adspent)"></td>
                <td class="px-3 py-2" x-text="num(row.orders)"></td>
                <td class="px-3 py-2" x-text="num(row.proceed)"></td>
                <td class="px-3 py-2" x-text="num(row.cannot_proceed)"></td>
