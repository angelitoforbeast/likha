<x-layout>
      ...
      <table id="supplyTable" class="min-w-full text-sm border-collapse">
        ...
        <tfoot>
          <tr style="background:#f8fafc; border-top:2px solid #94a3b8; font-weight:700;">
            <td class="px-3 py-2 border border-gray-300 text-xs text-gray-500 uppercase" colspan="1">TOTAL</td>
            <td class="px-3 py-2 border border-gray-300 text-right text-blue-700">{{ number_format($totalHoldUnits) }}</td>
            <td class="px-3 py-2 border border-gray-300" colspan="6"></td>
  ...
  <script>
    ...
    function recompute(row) {
      ...
      const recCell = row.querySelector('td[data-order]');
      if (recCell && recCell.querySelector('.rec-val')) recCell.dataset.order = recommended;
    }
    ...
    document.addEventListener('change', function (e) {
      ...
      if (e.target.classList.contains('running-chk')) {
        ...
        row.dataset.runningAuto = '0'; // manual override
        // Update "auto" label
        const autoLabel = row.querySelector('span.text-gray-400');
